...
//---------------------------------------------------------------------------
static ULONG _stdcall GetSizeOfImagePE(PVOID Header)
{
 DOS_HEADER *DosHdr = (DOS_HEADER*)Header;
 WIN_HEADER<PECURRENT> *WinHdr = (WIN_HEADER<PECURRENT>*)&(((BYTE*)Header)[DosHdr->OffsetHeaderPE]);
...
//---------------------------------------------------------------------------
template<typename T> static UINT _stdcall GetEntryPointersForApiName(char* ModuleName, char* ApiName, PVOID ModuleBase, T* *EntryA, T* *EntryB)
{
 if(!IsValidPEHeader(ModuleBase))return 1;
 DBGMSG("ModuleName='%s', ApiName='%s', ModuleBase=%p",ModuleName,ApiName,ModuleBase);
 ...
 for(UINT tctr=0;Import[tctr].AddressTabRVA;tctr++)
  {
   ...
   char* CurModlName        = (LPSTR)&((PBYTE)ModuleBase)[RvaToFileOffset(ModuleBase,Import[tctr].ModuleNameRVA,NULL)];
   if(ModuleName && lstrcmpiA(CurModlName,ModuleName))continue;
   DBGMSG("Module <%s>, LTable=%p, ATable=%p",CurModlName,LTable,ATable);
   for(UINT actr=0;Table[actr];actr++)
	{
	 if((Table[actr] & 0xFFFF0000)==0x80000000)continue;  // Skip imports by ordinal
     char* CurProcName = (Table[actr] > 103809024)?((LPSTR)(Table[actr]+2)):((LPSTR)&((PBYTE)ModuleBase)[RvaToFileOffset(ModuleBase,Table[actr]+2,NULL)]);  // May be already address, not offset  // 103809024 = Max offset
     //DBGMSG("Proc <%s>",CurProcName.Buffer);
     if(!lstrcmpA(CurProcName,ApiName))
      {
       if(EntryA)*EntryA = (LTable)?(&LTable[actr]):(NULL); 
       if(EntryB)*EntryB = (ATable)?(&ATable[actr]):(NULL); 
       DBGMSG("Found Import: <%s> %s",CurModlName,CurProcName);
       return 0;
      }
	}
  }   
...
//---------------------------------------------------------------------------
static UINT _stdcall OIGetEntryPointersForApiName(CHAR* ModuleName, CHAR* ApiName, PVOID ModuleBase, PVOID* EntryA, PVOID* EntryB)
{
 if(!IsValidPEHeader(ModuleBase))return 2;
 if(IsValidModuleX64(ModuleBase))return GetEntryPointersForApiName<PETYPE64>(ModuleName, ApiName, ModuleBase, (PETYPE64**)EntryA, (PETYPE64**)EntryB); 
 return GetEntryPointersForApiName<PETYPE32>(ModuleName, ApiName, ModuleBase, (PETYPE32**)EntryA, (PETYPE32**)EntryB); 
}
//---------------------------------------------------------------------------
template<typename T> static PVOID _stdcall TGetProcedureAddress(PVOID ModuleBase, char* ApiName)  // No forwarding support
{
 DOS_HEADER     *DosHdr;
 WIN_HEADER<T>  *WinHdr;
 EXPORT_DIR     *Export;
 ...
 for(DWORD ctr=0;ctr < Export->NamePointersNumber;ctr++)
  {
   char* CurProcName = (LPSTR)&((PBYTE)ModuleBase)[(((PDWORD)&((PBYTE)ModuleBase)[Export->NamePointersRVA])[ctr])];
   //DBGMSG("Proc <%s>",CurProcName);
   if(lstrcmpA(CurProcName,ApiName)==0)    // if(RtlEqualString(&CurProcName,ApiName,FALSE))
    {     
     ...
     DBGMSG("Found Export: <%s> %u:%p:%p",ApiName,Value,Entry,Addr);
	 return Addr;
    }  
  }
 return NULL;
...
//---------------------------------------------------------------------------
static PVOID _stdcall GetProcedureAddress(PVOID ModuleBase, char* ApiName) 
{
 DBGMSG("Entering...");
...
{
 return GetProcedureAddress(ModuleBase, ApiName);
}
//---------------------------------------------------------------------------
static PVOID _stdcall ModuleAddressToBase(PVOID Address)  // Not all module sections may present in memory (discarded)
{     
 PBYTE Base = (PBYTE)(((SIZE_T)Address) & ~0xFFF);   
 while(IsBadReadPtr(Base, sizeof(void*)) || !IsValidPEHeader(Base))Base -= 0x1000;  // !MmIsAddressValid(Base) // Why not all pages of ntoskrnl.exe are available on x64?
...
//---------------------------------------------------------------------------
static UINT _stdcall GetModuleSectionByIdx(PVOID ModuleBase, UINT SecIdx, SECTION_HEADER *ResSec) 
{
 DOS_HEADER     *DosHdr = (DOS_HEADER*)ModuleBase;
 WIN_HEADER<PECURRENT>  *WinHdr = (WIN_HEADER<PECURRENT>*)&((PBYTE)ModuleBase)[DosHdr->OffsetHeaderPE];
 if(SecIdx >= WinHdr->FileHeader.SectionsNumber)return 1;
 UINT            HdrLen = DosHdr->OffsetHeaderPE+WinHdr->FileHeader.HeaderSizeNT+sizeof(FILE_HEADER)+sizeof(DWORD);
 SECTION_HEADER *CurSec = (SECTION_HEADER*)&((BYTE*)ModuleBase)[HdrLen];
 RtlCopyMemory(ResSec,&CurSec[SecIdx],sizeof(SECTION_HEADER));
 return 0;
}
//---------------------------------------------------------------------------
